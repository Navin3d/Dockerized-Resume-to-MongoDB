...
function ProfilesList() {
  ...
  const initList = async (searchKey) => {
    const resonse = await axios.get(`${BASEURL}/skill/name/${searchKey}`);
    if (!resonse.data) {
      setData([]);
      setList([]);
    } else {
      setList(resonse.data.users);
